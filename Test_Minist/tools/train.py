...
import glob
from utils.labels_dict import UNI_UID2UNAME, CMP_FACADE_NAME
...
def getting_predictions(model, image: np.ndarray, gt_embs_list) -> np.ndarray:
    height, weight, _ = image.shape
    mean, std = get_imagenet_mean_std()
    crop_h = (np.ceil((height - 1) / 32) * 32).astype(np.int32)
    crop_w = (np.ceil((weight - 1) / 32) * 32).astype(np.int32)
    ...
    prediction_crop = logit_universal[:, pad_h_half:pad_h_half + height, pad_w_half:pad_w_half + weight]
...
def test_model(model, test_image_paths, test_mask_paths, device, save_dir, base_size = 128, batch_size = 10):
    ...
    while idx < len(test_image_paths):
        ...
        "using a new dictionary which I created for this task. The definitions were not taken from the dataset document"
        gt_embs_list = create_embs_from_names(classes, CMP_FACADE_NAME).float()

        for i in range(len(batch_images)):
            save_path = os.path.join(save_dir, test_image_paths[idx+i][38:])
            ...
            "transpose so the channels appear first"
            ...
            if os.path.exists(os.path.dirname(os.path.splitext(save_path)[0] + '_vis.png')) == False:
              os.makedirs(os.path.dirname(os.path.splitext(save_path)[0] + '_vis.png'))
    ...
    return float(test_f1.cpu().numpy()), float(test_accuracy.cpu().numpy()), float(test_recall.cpu().numpy())
...
def train(TRAIN_TEST_SPLIT = 0.8, dataset_path = '/content/SSIW-master/Test_Minist/base', ckpt_path = '/content/SSIW-master/Test_Minist/models/segformer_7data.pth',
    ...
    print(device)
    model = get_configured_segformer(n_classes=512, criterion=None, load_imagenet_model=False)
    ...
    model.to(device)
    print('model loaded')
    "we will use a very small learning rate since we are fine tuning"
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0005)
    "using dice loss for multi-class semantic segmentation, https://github.com/shruti-jadon/Semantic-Segmentation-Loss-Functions"
    ...
    "going through all dataset once per epoch"
    for i in range(epochs):
        ...
        while idx < len(train_image_paths):
            ...
            for j in range(len(batch_images)):
                ...
                logit = getting_predictions(model, batch_images[j], gt_embs_list=gt_embs_list)
        ...
        print(batches)
        print(f'epoch {i+1} loss = {epoch_loss/len(train_image_paths)} ')
        training_loss.append(epoch_loss/len(train_image_paths))
        save_dir_name = '/content/SSIW-master/Test_Minist/epoch_results/epoch'+str(i+1)
        if os.path.exists(save_dir_name) == False:
            os.mkdir(save_dir_name)
        f1, accuracy, recall = test_model(model, test_image_paths, test_mask_paths, device, save_dir_name)
        test_f1.append(f1)
        test_accuracy.append(accuracy)
        test_recall.append(recall)

        torch.save(model.state_dict(), f'/content/SSIW-master/Test_Minist/models/epoch{i+1}.pth')
        print("Training Loss: ")
        print(training_loss)
